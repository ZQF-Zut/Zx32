...
static auto TestStr()
{
    // mbcs to widestr
    {
        Wx32::WideStr_t wide_str = Wx32::Utils::StrCvtToWide(reinterpret_cast<const char*>(u8"这是一段中文"), Wx32::CodePage::UTF8);
    ...
    // widestr to mbcs
    {
        Wx32::MbcsStr_t mbc_str0 = Wx32::Utils::StrCvtToMbcs(L"这是一段中文", Wx32::CodePage::UTF8);
        if (mbc_str0.first != reinterpret_cast<const char*>(u8"这是一段中文"))
        {
            return false;
        }

        auto mbc_str1 = Wx32::Utils::StrCvtToMbcs(L"这是一段中文", Wx32::CodePage::UTF8);
        if (mbc_str1.first != reinterpret_cast<const char*>(u8"这是一段中文"))
        {
            return false;
        }
    }
    ...
    // try error covert
    {
        Wx32::MbcsStr_t mbcs_str = Wx32::Utils::StrCvtToMbcs(L"这是一段UTF16的中文字符, 但使用SJIS转换, 肯定丢失字符", Wx32::CodePage::SJIS);
        Wx32::WideStr_t wide_str = Wx32::Utils::StrCvtToWide(reinterpret_cast<const char*>(u8"这是一段UTF8的中文字符, 但使用GBK转换, 肯定会出错"), Wx32::CodePage::GBK);
    }
